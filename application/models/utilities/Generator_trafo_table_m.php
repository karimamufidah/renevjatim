...
class Generator_trafo_table_m extends CI_Model
{
    ...
    {
        return "COALESCE((SELECT eval_$pukul FROM trafo_realisasi WHERE tanggal = '$params->tanggal' AND trafo = '$params->nama' AND satuan = 'MW' LIMIT 1), 0)";
    }

    private function _generate_mx_query($params, $pukul)
    {
        return "COALESCE((SELECT eval_$pukul FROM trafo_realisasi WHERE tanggal = '$params->tanggal' AND trafo = '$params->nama' AND satuan = 'MX' LIMIT 1), 0)";
    }

    private function _generate_percentage_query($params, $pukul)
    {
        return "COALESCE((SELECT eval_$pukul FROM trafo_realisasi WHERE tanggal = '$params->tanggal' AND trafo = '$params->nama' AND satuan = '%' LIMIT 1), 0)";
    }
