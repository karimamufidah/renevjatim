<?php
class Tegangan_realisasi_table_m extends CI_Model
{
  /** Index */
  public function index($offset, $limit, $q, $order_column, $order_direction, $filters)
  {
    $this->db->select("main.logged_at, main.nama, COALESCE(main.kv, 0) AS kv, COALESCE(main.percentage, 0) AS percentage");
  ...
  private function _where_query($filters)
  {
    ...
    $this->db->where('main.kv >=', $filters->persentase_awal);
    $this->db->where('main.kv <=', $filters->persentase_akhir);
  }
