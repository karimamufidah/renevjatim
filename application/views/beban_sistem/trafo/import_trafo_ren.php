<div class="container-xxl flex-grow-1 container-p-y">
  <h4 class="fw-bold py-3 mb-4"><a href="<?= site_url('beban_sistem/trafo_perencanaan') ?>"><span class="text-muted fw-light">Beban Sistem /</span></a> Trafo TD & KTT | Perencanaan</h4>

  <form id="formBulkUpload" method="POST" onsubmit="checkDate();return false;" action="<?= site_url('beban_sistem/upload_trafo_ren') ?>" enctype="multipart/form-data">
    <div class="card mb-4">
    ...
    <div class="mt-2">
      ...
      <a href="<?= site_url('beban_sistem/trafo_perencanaan') ?>" class="btn btn-outline-secondary">Cancel</a>
    </div>
  </form>
</div>
...
<script>
  const urlAPICountByDate = "<?= base_url("api/utilities/get-beban-trafo-perencanaan-by-date-count"); ?>";
  const urlAPIUpload = "<?= base_url("api/import/trafo-perencanaan/store"); ?>";
  const INSERT = 1;
  const UPDATE = 2;
  let crud;
    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
          ...
          $("#modal").modal("hide");
          setTimeout(function() {
            window.open("<?= base_url("beban_sistem/trafo_perencanaan"); ?>", "_self");
          }, 2000);
