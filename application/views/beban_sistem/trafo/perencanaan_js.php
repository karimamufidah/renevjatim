...
<script>
  ...
  const urlAPIDeleteByDate = "<?= base_url("api/utilities/delete-trafo-perencanaan-by-date"); ?>";
  ...
  async function initializeSelect2Default() {
    await getDefaultSelect2({
      id: 'f-unit-default',
      url: `${urlAPITransformerUnitFilterSelect2}`
    });
  }
  ...
  function generateInfo(data) {
    return `
      ...
      <strong style="text-transform:uppercase">${data.trafo}</strong><br>
  ...
  function generateButtons(data) {
    return `
      ${generateEditButtonLink(`<?= base_url('beban_sistem/edit_trafo_ren'); ?>/${data.data_id}`)}
      ${generateDeleteButtonLink(`<?= base_url('beban_sistem/del_trafo_ren'); ?>/${data.data_id}`)}
    `;
  }
