...
<script>
  ...
  const urlAPIDeleteByDate = "<?= base_url("api/utilities/delete-sistem-perencanaan-by-date"); ?>";
  ...
  function generateInfo(data) {
    return `
      ...
      <strong style="text-transform:uppercase">${data.sistem}</strong><br>
  ...
  function generateButtons(data) {
    return `
      ${generateEditButtonLink(`<?= base_url('beban_sistem/edit_sistem_ren'); ?>/${data.data_id}`)}
      ${generateDeleteButtonLink(`<?= base_url('beban_sistem/del_sistem_ren'); ?>/${data.data_id}`)}
    `;
  }
