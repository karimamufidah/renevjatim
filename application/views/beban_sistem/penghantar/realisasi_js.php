...
<script>
  const urlAPIMain = "<?= base_url("api/penghantar-realisasi"); ?>";
  const urlAPICountByDate = "<?= base_url("api/utilities/get-beban-penghantar-realisasi-by-date-count"); ?>";
  const urlAPIDeleteByDate = "<?= base_url("api/utilities/delete-penghantar-realisasi-by-date"); ?>";
  const urlAPIDeliveryUnitFilterSelect2 = "<?php echo base_url('api/select2/satuan-penghantar-filter'); ?>";
  const urlAPIMainDatatable = "<?php echo base_url('api/datatable/penghantar-realisasi'); ?>";
  ...
  function generateInfo(data) {
    return `
      ...
      <strong style="text-transform:uppercase">${data.ruas}</strong><br>
  ...
  function generateDetail(data) {
    return `
      <div id="detail-${data.data_id}" style="display:none">
        ...
        <small>Numerik: ${data.numerik}</small><br>
  ...
  function generateButtons(data) {
    return `
      ${generateEditButtonLink(`<?= base_url('beban_sistem/edit_penghantar_eval'); ?>/${data.data_id}`)}
      ${generateDeleteButtonLink(`<?= base_url('beban_sistem/del_penghantar_eval'); ?>/${data.data_id}`)}
    `;
  }
