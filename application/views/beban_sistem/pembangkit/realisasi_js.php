...
<script>
  const urlAPIMain = "<?= base_url("api/pembangkit-realisasi"); ?>";
  const urlAPICountByDate = "<?= base_url("api/utilities/get-beban-pembangkit-realisasi-by-date-count"); ?>";
  const urlAPIDeleteByDate = "<?= base_url("api/utilities/delete-pembangkit-realisasi-by-date"); ?>";
  const urlAPIGeneratorUnitFilterSelect2 = "<?php echo base_url('api/select2/satuan-pembangkit-filter'); ?>";
  const urlAPIMainDatatable = "<?php echo base_url('api/datatable/pembangkit-realisasi'); ?>";
  ...
  function generateButtons(data) {
    return `
      ${generateEditButtonLink(`<?= base_url('beban_sistem/edit_pembangkit_eval'); ?>/${data.data_id}`)}
      ${generateDeleteButtonLink(`<?= base_url('beban_sistem/del_pembangkit_eval'); ?>/${data.data_id}`)}
    `;
  }
