...
<script>
  const urlAPIMain = "<?= base_url("api/interbus-transformer-realisasi"); ?>";
  const urlAPICountByDate = "<?= base_url("api/utilities/get-beban-ibt-realisasi-by-date-count"); ?>";
  const urlAPIDeleteByDate = "<?= base_url("api/utilities/delete-ibt-realisasi-by-date"); ?>";
  const urlAPIInterbusTransformerUnitFilterSelect2 = "<?php echo base_url('api/select2/satuan-interbus-transformer-filter'); ?>";
  const urlAPIMainDatatable = "<?php echo base_url('api/datatable/interbus-transformer-realisasi'); ?>";
  ...
  async function initializeSelect2Default() {
    await getDefaultSelect2({
      id: 'f-unit-default',
      url: `${urlAPIInterbusTransformerUnitFilterSelect2}`
    });
  }
  ...
  function generateInfo(data) {
    return `
      ...
      <strong style="text-transform:uppercase">${data.ibt}</strong><br>
  ...
  function generateButtons(data) {
    return `
      ${generateEditButtonLink(`<?= base_url('beban_sistem/edit_ibt_eval'); ?>/${data.data_id}`)}
      ${generateDeleteButtonLink(`<?= base_url('beban_sistem/del_ibt_eval'); ?>/${data.data_id}`)}
    `;
  }
  ...
  async function postData(data) {
    const response = await crud.create({
      url: urlAPIMain,
      data: data,
      name: "realisasi interbus tranformer"
    });
