...
class Xlsx_subsistem extends CI_Controller
{
  ...
  {
    if (!isset($request->subsistem)) $request->subsistem = null;
    if (!isset($request->pasokan)) $request->pasokan = null;
  ...
  {
    $this->load->model("export/get_subsistem_perencanaan_by_date_m", "plan");
    $this->load->model("export/get_subsistem_realisasi_by_date_m", "realization");
  ...
  private function _get_data_detail($filters)
  {
    $realization = $this->realization->show((object) array(
      "subsistem" => $filters->subsistem,
      "pasokan" => $filters->pasokan,
  ...
  {
    $sheet->setCellValue("A$request->index", $request->subsistem);
    $sheet->setCellValue("B$request->index", $data->date);
    $sheet->setCellValue("C$request->index", $request->pasokan);
  ...
  private function _download_spreadsheet($request)
  {
    ...
    $name = "subsistem-harian-" . str_replace(" ", "", str_replace("-", "", str_replace(":", "", generate_timezone_timestamp(null))));
