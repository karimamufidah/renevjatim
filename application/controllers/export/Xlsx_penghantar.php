...
class Xlsx_penghantar extends CI_Controller
{
  ...
  {
    if (!isset($request->ruas)) $request->ruas = null;
  ...
  {
    $this->load->model("export/get_penghantar_perencanaan_by_date_m", "plan");
    $this->load->model("export/get_penghantar_realisasi_by_date_m", "realization");
  ...
  private function _get_data_detail($filters)
  {
    ...
    if (!$filters->isWithPlan) {
      ...
      if (!$plan) $plan = $this->_generate_empty_realization();
    }
  ...
  {
    $sheet->setCellValue("A$request->index", $request->ruas);
  ...
  private function _download_spreadsheet($request)
  {
    ...
    $name = "penghantar-harian-" . str_replace(" ", "", str_replace("-", "", str_replace(":", "", generate_timezone_timestamp(null))));
